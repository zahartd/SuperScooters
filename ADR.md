...
- **Расчёт нагрузки и требования по CPU, RAM и диску**:  
  ...
  - RAM: 512 МБ–1 ГБ под runtime и коннекты к БД users, берем с запасом 4 Гб.  
  - Диск: только логи/образ - 20 гб; данные в БД users (отдельный инстанс).
  - Итого: 1 инстанс с 2 vCPU, 4гб RAM, 20 ГБ SSD.
- **Логика работы**:  
...
- **Деградация**: при недоступности сервиса возвращаем консервативную стратегию: повышенный тариф/мультипликатор, требование залога и обязательный payment pre-check; возможность глобально блокировать новых/рискованных пользователей.
