...
#### HTTP API сервис
- **Подробное описание**: FastAPI-сервис с ручками `/offers`, `/orders`, `/orders/{id}`, `/orders/{id}/finish`; слои: http routing, доменные use-cases, репозитории (PostgreSQL + кеш), HTTP-клиенты к внешним сервисам, вспомогательные utils. Stateless для `/offers`, stateful для `/orders`.
- **Расчёт нагрузки и требования по CPU, RAM и диску**:  
  - RPS: POST `/offers` ~40 (0.5*X*2), POST `/orders` ~20 (X), POST `/orders/{id}/finish` ~20 (X), GET `/orders/{id}` ~1000 (X*Y=20*50). Итог: ~80 write-RPS и ~1000 read-RPS на один инстанс.  
  - CPU: считаем CPU-bound часть ~3 мс на запрос (валидация+JWT+логика; I/O вынесено во внешние вызовы/БД). При 1080 RPS это ~3.2 CPU-сек/сек → укладывается в 4 vCPU с ~80% загрузкой и запасом на пики. Стартуем **1 инстанс** 4 vCPU; масштабируем до 2–3 при росте RPS или удлинении логики.  
  - RAM: под код/обработчики/клиентские пуулы ~1–1.5 ГБ, плюс буферы и метрики; при 1 инстансе берём 12 ГБ как бюджет с запасом под временные аллокации.  
  - Диск: только логи/образ, 5–10 ГБ SSD на инстанс достаточно; состояние не хранится.
- **Логика работы**:  
...
- **Альтернативы (C + / - и решениями)**:  
  - Держать логику долгов/репутации внутри основного сервиса. **Плюсы**: меньше внешних вызовов. **Минусы**: нет изоляции и отдельного SLA, сложнее эволюция правил. **Решение**: используем вынесенный сервис репутации; вернём внутрь только если RPS или задержки сервиса станут bottleneck.  
...
- **Деградация**: при падении узла — теряем все ручки, поэтому минимум 2 инстанса за балансировщиком; отключаем дорогостоящие внешние вызовы через feature-флаги, уменьшаем TTL кешей чтобы быстрее расходились ошибки, быстрый отказ (503) при недоступности критичных зависимостей, fallback на статические конфиги для некритичных данных.
...
- **Расчёт нагрузки и требования по CPU, RAM и диску**:  
  - Writes: `orders` ~20 INSERT + 20 UPDATE = ~40 оп/сек, `user_summary` до ~40 UPDATE/сек → ~80 write/сек (≈800 IOPS при WAL/индексах, fits в 1 узел).  
  ...
  - Объём: X=20 → ~1.7M заказов/день (~16.5 ГБ/день), ~0.5 ТБ за 30 дней горячих данных, ~5.8 ТБ/год, ~29 ТБ за 5 лет; с индексами/бэкапами берём бюджет архива ~90 ТБ.  
  - Конфигурация и инстансы: стартуем **1 primary** 8 vCPU, 32 ГБ RAM, NVMe ~32 ТБ, партиционирование по дню/неделе. При росте — добавить 1–2 реплики на чтение и масштабировать storage/CPU.  
...
- **Логика работы**: ACID-транзакции при создании/завершении, идемпотентность через уникальные ключи/idempotency key, партиционирование `orders` по времени, регулярный вакуум/maintenance, TTL/архивация данных старше 30 дней во внешнее хранилище или холодные партиции, индексы по `order_id`, `user_id`, `created_at`.
- **Альтернативы (C + / - и решениями)**:  
...
- **Расчёт нагрузки и требования по CPU, RAM и диску**:  
  - Для заказов: 20 RPS * ttl_sec * 5 КБ * 1.2 → ttl 20/30/60 мин ≈ 144/216/432 МБ; при росте до 40 RPS и ttl 30 мин — ~432 МБ.  
  - Зоны: 5–10 КБ на запись, 5k–10k зон → 10–20 МБ. Конфиги: <1 МБ.  
  - Итого по узлу: ~227 МБ при 20 RPS и ttl 30 мин; ~452 МБ при ttl 60 мин; резервируем ≥512 МБ на кеш. CPU — O(1) операции, пренебрежимо; диск не используется. Один кеш-инстанс на каждый инстанс приложения (локально).
...
#### Сервис репутации пользователей (risk/billing)
- **Подробное описание**: отдельный HTTP-сервис поверх БД `users`, держит одну профильную таблицу (например `user_risk_profile`) с полями долгов, количества поездок, флагов риска. Отдаёт параметры для оффера (мультипликатор цены, нужен ли залог, нужен ли pre-check, можно ли ехать), обновляет профиль после завершения поездки.
- **Расчёт нагрузки и требования по CPU, RAM и диску**:  
  - RPS: ~40 read-RPS на `/profile` (каждый оффер), ~20 read-RPS на старте заказа (повторная проверка), ~20 write-RPS на завершении заказа → всего ~60 reads + ~20 writes = ~80 RPS на **1 инстанс**.  
  - CPU: простой JSON + SQL, оцениваем 1–2 мс CPU на запрос → ~0.16 CPU-сек/сек; достаточно 2 vCPU с большим запасом.  
  - RAM: 512 МБ–1 ГБ под runtime и коннекты к БД users.  
  - Диск: только логи/образ; данные в БД users (отдельный инстанс или отдельная схема).
- **Логика работы**:  
  - `GET /profile`: по `user_id` возвращает параметры риска (мультипликатор цены, нужен ли залог, нужен ли payment pre-check, можно ли ехать).  
  - `POST /profile/update`: инкрементирует счётчики поездок, обновляет долги после завершения заказа.  
  - Может кешировать горячие профили in-memory для снижения латентности.
- **Альтернативы (C + / - и решениями)**:  
  - Держать данные в основном сервисе. **Плюсы**: минус сеть. **Минусы**: нет изоляции и отдельного SLA. **Решение**: остаёмся на выделенном сервисе.  
  - Сложный скоринг/ML сервис. **Плюсы**: точнее риски. **Минусы**: сложнее поддержка и вычисления. **Решение**: MVP — простая таблица и правила; ML обсудим при росте.  
- **Деградация**: при недоступности сервиса возвращаем консервативную стратегию: повышенный тариф/мультипликатор, требование залога и обязательный payment pre-check; возможность глобально блокировать новых/рискованных пользователей. Логируем отказы, быстрый отказ по таймауту/circuit-breaker.
...
- **Логика работы**:  
  - Старт: читаем/бронируем самокат (`scooters`), делаем платежный pre-check (`payments`), подтягиваем профиль `users`, применяем тарифы `zones`/`configs` и параметры риска от `reputation` (с кешом и статическим fallback).  
  ...
  - Чтения `/orders/{id}` наружу не ходят. Таймауты/ретраи/ circuit-breaker на write-пути.
- **Альтернативы (C + / - и решениями)**:  
  ...
  - Асинхронные платежи (очередь вместо синхронного вызова). **Плюсы**: меньше блокировок. **Минусы**: сложнее UX/согласование статусов. **Решение**: оставляем синхронно, очередь добавить как fallback для задержек.  
  - Мок/стаб для `users`/`zones` на время недоступности. **Плюсы**: повышаем доступность. **Минусы**: риск неточных данных. **Решение**: используем кеш + статический конфиг вместо полноценного стаба.
- **Деградация**:  
  ...
  - Общие меры: таймауты, ограничение ретраев, circuit-breaker, логирование причин отказа.

---

## 4. Развёртывание и миграции

- Docker + docker-compose поднимает `app`, `postgres`, локальный cache/sidecar; volumes под данные/бэкапы, healthcheck и зависимость `app -> postgres`.
- Миграции БД отдельным шагом/контейнером (`psql` или `alembic upgrade`), idempotent-скрипты с версированием и логированием.
- Конфиги через env/.env; для prod — secrets/CI vars, статические fallback-конфиги (zones/configs) лежат в образе/volume.
- Старт на одной машине (compose/копирование образа), далее переход на docker swarm или k8s для нескольких узлов и rolling/zero-downtime.
