...
def create_offer(request: OfferRequest):
    logger.info("api: POST /offers scooter_id=%s user_id=%s", request.scooter_id, request.user_id)
    ...
    if isinstance(result, offers_service.CreateOfferError):
        logger.warning("api: create_offer failed user_id=%s reason=%s", request.user_id, result.message)
        return OfferResponse(error=result.message)
    offer, token = result
    logger.info("api: create_offer success offer_id=%s user_id=%s", offer.id, request.user_id)
...
def create_order(request: OrderStartRequest, conn: Connection = Depends(get_connection)):
    try:
        logger.info(
            "api: POST /orders offer_id=%s user_id=%s scooter_id=%s",
            request.offer.id,
            request.offer.user_id,
            request.offer.scooter_id,
        )
        order = orders_service.start_order(request.offer.to_dataclass(), request.pricing_token, conn, static_config.clone())
    except ValueError as exc:
        logger.warning("api: create_order validation failed detail=%s", str(exc))
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    logger.info("api: create_order success order_id=%s user_id=%s", order.id, order.user_id)
...
def finish_order(order_id: str, conn: Connection = Depends(get_connection)):
    try:
        logger.info("api: POST /orders/%s/finish", order_id)
        order = orders_service.finish_order(order_id, conn, static_config.clone())
    except KeyError:
        logger.warning("api: finish_order not found order_id=%s", order_id)
        raise HTTPException(status_code=404, detail="order not found")
    logger.info("api: finish_order success order_id=%s", order_id)
    return OrderResponse.from_dataclass(order)


@router.get("/orders/{order_id}", response_model=OrderResponse)
def get_order(order_id: str, conn: Connection = Depends(get_connection)):
    logger.info("api: GET /orders/%s", order_id)
    order = orders_service.get_order(order_id, conn, static_config.clone())
    if order is None:
        logger.warning("api: get_order not found order_id=%s", order_id)
        raise HTTPException(status_code=404, detail="order not found")
    logger.info(
