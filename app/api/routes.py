...
def create_order(request: OrderStartRequest):
    try:
        order = orders_service.start_order(request.offer.to_dataclass(), request.pricing_token)
...
def finish_order(order_id: str):
    try:
        order = orders_service.finish_order(order_id)
    except KeyError:
